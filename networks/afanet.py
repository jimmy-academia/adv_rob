...
class AFANet(nn.Module):
    def __init__(self, args, base_filter_channels=16, additional_layers=3):
        ...
        self.base_filter_channels = base_filter_channels
        self.additional_layers = additional_layers

        self.base_predictor = nn.Sequential(*self._make_layers(1))
        self.pat_predictor = nn.Sequential(*self._make_layers(args.vocab_size))
    ...
    def _make_layers(self, _final_channel):
        ...
        while True:
            ...
            reduction_done *= 2
            if s == 1: 
                for __ in range(self.additional_layers):
    ...
    def forward(self, x):
        b = self.base_predictor(x)  # bs, 1, 16, 16
        b = b.repeat(1, 3, 1, 1)
        b = b.repeat_interleave(2, dim=2).repeat_interleave(2, dim=3) 

        x = self.pat_predictor(x)     # bs, T, 16, 16
        ...
        gamma = 0.01
        # gamma = self.args.eps if self.args.eps is not None else 0.01
        x = gamma * x + b
