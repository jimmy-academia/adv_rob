...
class PatchSquareAttack():
    ...
    def __init__(
            self, predict, norm='Linf', n_queries=5000, eps=None, p_init=.8, n_restarts=1, seed=0, verbose=False, targeted=False, loss='margin', resc_schedule=True, device=None):
        ...
        self.targeted = False # don't consider targeted...
        self.loss = loss
    ...
    def margin_and_loss(self, x, y):
        ...
        >> adjusted for patches
        x is images
        y is tokens predictions (Batch_size, num_patches)
        """

        logits = self.predict(x)
        # xent = F.cross_entropy(logits, y, reduction='none')
        u = torch.arange(logits.shape[0]).unsqueeze(1).expand_as(y)
        v = torch.arange(logits.shape[1]).unsqueeze(0).expand_as(y)
        y_corr = logits[u, v, y].clone()
        logits[u, v, y] = -float('inf')
        y_others = logits.max(dim=-1)[0]

        # max margin for multi patches per image becomes negative when all is negative
    ...
    def perturb(self, x, y=None):
        ...
        if not self.targeted:
            acc = (self.predict(x).max(2)[1] == y).all(1)
        else:
        ...
        for counter in range(self.n_restarts):
            ...
            if ind_to_fool.numel() != 0:
                ...
                if not self.targeted:
                    acc_curr = (output_curr.max(2)[1] == y_to_fool).all(1)
