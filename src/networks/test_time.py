...
class TTTBasicModel(nn.Module):
    '''
    extractor (feature extractor)
    rotatehead (rotation prediction head for self-supervised learning)
    classhead (main classification head)
    '''
    def __init__(self, args):
        super(TTTBasicModel, self).__init__()
        self.args = args
        
        # use deepcopy to get independent layers
        if args.dataset == 'imagenet':
            self.net = models.resnet18(weights=None)
            self.net.fc = nn.Linear(512, args.num_classes)
            _layers = [self.net.conv1, self.net.bn1, self.net.relu, self.net.maxpool, self.net.layer1, self.net.layer2, self.net.layer3]
            _ind_layers = copy.deepcopy([self.net.layer4, self.net.avgpool, ViewFlatten(), nn.Linear(512, 4)])
        else:
            self.net = ResNetCifar(depth=26, classes=args.num_classes)
            _layers = [self.net.conv1, self.net.layer1, self.net.layer2]
            _ind_layers = copy.deepcopy([self.net.layer3, self.net.bn, self.net.relu, self.net.avgpool, ViewFlatten(), nn.Linear(64, 4)])

        self.extractor = nn.Sequential(*_layers)
        self.rotatehead = nn.Sequential(*_ind_layers)

    def forward(self, x):        
        return self.net(x)

    def sshead(self, x):
        features = self.extractor(x)  # Common feature extractor
        rotation_output = self.rotatehead(features)
        return rotation_output


### ========= from TTT source code ============== ###
